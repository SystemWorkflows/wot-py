...
_logger = logging.getLogger("coffee-machine")
...
async def main():
   # _logger.info("Creating WebSocket server on: {}".format(WEBSOCKET_PORT))
    ...
    _logger.info("Exposing and configuring Thing")
    ...
    await exposed_thing.properties["possibleDrinks"].write(
    ...
    exposed_thing.set_property_write_handler(
        "servedCounter", served_counter_write_handler
    )

    # Now initialize the servedCounter property
    await exposed_thing.properties["servedCounter"].write(
        ...
        for resource, value in newResources.items():
            if value <= 0:
                ...
                exposed_thing.emit_event(
                    "outOfResource", f"Low level of {resource}: {resources[resource]}%"
                )
    ...
    _logger.info(f'{TD["title"]} is ready')
